...
export function PqcSignSubmit({ onNext, onBack }: { onNext: () => void, onBack: () => void }) {
  ...
  const [builtUserOp, setBuiltUserOp] = React.useState<UserOperationV07 | null>(null)
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (!accountAddr) throw new Error("No deployed PQC account found. Go back to Deploy.")
      if (!keypair) throw new Error("No PQC keypair found. Go back to Generate.")
      ...
      const buildSignedUserOp = async (preVerificationGas: bigint): Promise<{ userOp: UserOperationV07; userOpHash: `0x${string}` }> => {
        const unsigned: UserOperationV07 = {
          sender: accountAddr as `0x${string}`,
          nonce: viemToHex(BigInt(onChainNonce as bigint)),
          initCode: "0x",
          callData: callData as `0x${string}`,
          accountGasLimits: packAccountGasLimits(CALL_GAS, VERIFICATION_GAS),
          preVerificationGas: viemToHex(preVerificationGas),
          gasFees: packGasFees(maxFee, maxPrio),
          paymasterAndData: "0x",
          signature: "0x",
        }
        const userOpHash = getUserOpHashV07(unsigned, 11155111, ADDRESSES.entryPointV07)
        setSignStatus(`Signing ${wizard.pqc4337.scheme} in browser...`)
        const signature = wizard.pqc4337.scheme === "falcon-eth"
      ...
      let signed = await buildSignedUserOp(DEFAULT_PRE_VERIFICATION_GAS)
      if (pimlicoKey) {
        setSignStatus("Calibrating preVerificationGas...")
        ...
        const required = BigInt(est.preVerificationGas)
        const buffered = addPreVerificationGasHeadroom(required)
        if (buffered > BigInt(signed.userOp.preVerificationGas)) {
          signed = await buildSignedUserOp(buffered)
        }
      }

      setSignStatus("Pre-flight: verifying userOpHash against EntryPoint v0.7...")
      try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (publicClient) {
        const bal = await publicClient.getBalance({ address: accountAddr as `0x${string}` })
        if (bal === BigInt(0)) throw new Error("Account has 0 ETH. Fund it first.")
      }
      ...
      if (useBundler) {
        ...
        if (!walletAddress || !publicClient) throw new Error("Wallet client not ready.")
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Build UserOperation, sign in browser, and submit</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-border">
            ...
            <p className="text-xs text-muted">Scheme: <span className="text-foreground font-medium">{wizard.pqc4337.scheme}</span></p>
            <Button variant="outline" className="w-full" onClick={handleSign} disabled={isSigning || !!signatureHex}>
              {isSigning ? <div className="w-4 h-4 mr-2 border-2 border-accent border-t-transparent rounded-full animate-spin" /> : <KeyRound className="w-4 h-4 mr-2" />}
              {isSigning ? "Signing..." : signatureHex ? "Signed Successfully" : "Sign UserOperation"}
            </Button>
